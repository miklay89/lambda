...
// all clinics information in country
const allClinicsInfo: RequestHandler = async (req, res) => {
  try {
    const db = await dbObject.Connector.connect();
    const query = await db.select(clinicsTable).fields({
